...
internal class XamlXNameResolver : INameResolver, IXamlAstVisitor
{
    ...
    public XamlXNameResolver(DefaultFieldModifier defaultFieldModifier = DefaultFieldModifier.Friend)
    {
        _defaultFieldModifier = defaultFieldModifier.ToString();
    }
        ...
        foreach (var child in objectNode.Children)
        {
                ...
                propertyValueNode.Values[0] is XamlAstTextNode text)
            {
                ...
                var genericTypeName = typeAgs.Count == 0
                    ? $"Global.{typeName}"
                    : $@"Global.{typeName}(Of ({string.Join(", ", typeAgs.Select(arg => $"{arg}"))}))";
